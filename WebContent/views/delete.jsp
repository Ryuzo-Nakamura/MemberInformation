...
<form action="/MemberInformation/servlet/MM02search" method="post" id="searchForm">
	<p>■会員番号</p>
	<p>
		<input type="hidden" name="URI" value="<%=request.getRequestURI() %>">
		<input type="text" name="member_id" id="member_id" value="<c:if test="${member_id != null}">${member_id}</c:if>" maxlength="13" form="searchForm">
		<input type="submit" class="search_btn" value="検索" >
	</p>
	<p class="error">${search_message}</p>
</form>
<form action="/MemberInformation/servlet/MM03delete" method="post" id="updateForm">
	<h4>
		<c:choose>
			<c:when test="${search_message == null}">会員番号：${member_id}</c:when>
			<c:otherwise>会員番号を検索してください</c:otherwise>
		</c:choose>
	</h4>
	<input type="hidden" name="member_id" value="<c:if test="${member_id != null}">${member_id}</c:if>">
	<p>■名前</p>
	姓 <input type="text" name="last_name" id="last_name" value="<c:if test="${last_name != null}">${last_name}</c:if>" maxlength="32" readonly> 
	名 <input type="text" name="first_name" id="first_name" value="<c:if test="${first_name != null}">${first_name}</c:if>" maxlength="32" readonly>
	<p>■性別</p>
	<c:choose>
	...
	<p>■生年月日</p>
	<select name="birth_year" id="birth_year" disabled>
		<option value=""></option>
	...
	<select name="birth_month" id="birth_month" disabled>
		<option value=""></option>
		<% for(int m = 1; m <= 12; m++) { %>
	...
	<select name="birth_day" id="birth_day" disabled>
		<option value=""></option>
		<% for(int d = 1; d <= 31; d++) { %>
	...
	<p>■電話番号</p>
	<input type="tel" name="phone_number" id="phone_number" pattern="\d{2,4}-?\d{2,4}-?\d{3,4}" placeholder="ハイフン省略可" value="<c:if test="${phone_number != null}">${phone_number}</c:if>" maxlength="32" readonly>
	<p>■メールアドレス</p>
	<input type="email" name="mail_address" id="mail_address" value="<c:if test="${mail_address != null}">${mail_address}</c:if>" maxlength="32" readonly>
	<p>■職業</p>
	...
	<p class="error">${delete_message}</p>
	<p class="btn_p">
		<input type="submit" value="削除">
		<input type="button" value="戻る" onclick="MM03back()">
	</p>
</form>
